...
select * from employees where commision is null or commision=0;
...
select first_name,last_name,year(hire_date) as hire_date from employees;
...
select e.first_name,e.last_name, j.job_title, l.location from employees e join jobs j on e.job_id=j.job_id join departments d on e.department_id=d.department_id join location l on d.location_id=l.location_id;
...
select e.first_name,e.last_name,e.job_id from employees e join employees e join employees m on e.manager_id=m.employee_id where e.job_id=m.job_id;
#Display the names of employees who worked in different jobs in the past (use job_history)
select distinct e.first_name,e.last_name from employees e join job_history jh on e.employee_id=jh.employee_id where e.job_id<>jh.job_id;
